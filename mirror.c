...
//A mirror server function that creates a socket, binds it, listens for incoming connections, and then launches a new process to service the client.
int main() {
    ...
    var4SrvrAddLoc.sin_port = htons(RPPMCRO_cncPort4Mirror);
    ...
    // Mirror server is now active, waiting for clients
    printf("**Mirror Active for Clients Connection on the port: %d...\n", RPPMCRO_cncPort4Mirror);
    ...
    while (1) {
        ...
        // Recieve client counter from client
        recv(param4clntSckt, &var4ClntCounter, sizeof(var4ClntCounter), 0);
